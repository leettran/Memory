...
var selectedPicIdPart2Task1Rep;
var willCorrectTask1Rep = false;
var picSelectedPart2Task1Rep = false;
...
// shows pages after each other
function showPagesSuccessively(firstPage, secondPage) {
...
// shows given page immediately
function showFollowingPageImmediately(nextPage) {
...
function switchToNextTask6() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showFollowingPageImmediately('tableWithPicsTask6Rep');
            canSwitchToNextTask = false;
            picSelectedPart1 = false;
...
// switches to next task
 function switchToNextTask5Rep() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showFollowingPageImmediately('tableWithPicsTask6');
            canSwitchToNextTask = false;
            picSelectedPart1 = false;
...
function switchToNextTask5() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showFollowingPageImmediately('tableWithPicsTask5Rep');
            canSwitchToNextTask = false;
            picSelectedPart1 = false;
...
// switches to next task
 function switchToNextTask4Rep() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showFollowingPageImmediately('tableWithPicsTask5');
            canSwitchToNextTask = false;
            picSelectedPart1 = false;
...
function switchToNextTask4() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
           showFollowingPageImmediately('tableWithPicsTask4Rep');
            canSwitchToNextTask = false;
            picSelectedPart1 = false;
...
function switchToNextTask3Rep() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showPagesSuccessively('overThreeImagesHint', 'tableWithPicsTask4');
...
// switches to next task
  function switchToNextTask3() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showPagesSuccessively('threeImagesHint', 'tableWithPicsTask3Rep');
...
function switchToNextTask2() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showPagesSuccessively('twoImagesHint', 'tableWithPicsTask2Rep');
...
// switches to next task
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showPagesSuccessively('threeImagesHint', 'tableWithPicsTask3');
...
function switchToNextTask1() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showPagesSuccessively('oneImageHint', 'tableWithPicsTask1Rep');
...
function switchToNextTask1Rep() {
    ...
    try
    {
        if (canSwitchToNextTask)
        {
            showPagesSuccessively('twoImagesHint', 'tableWithPicsTask2');
...
function selectPicPart2Trial(clickedPic, choiceTableId) {
    ...
    try
    {
        ...
        else if (!willCorrect && hasPic)
        {
            ...
            if (currentClass !== "selectedPic" && !picSetted)
            {
                // select pic
...
function selectPicPart2Task1Rep(clickedPic, choiceTableId) {
    ...
    try
    {
        ...
        if (willCorrectTask1Rep && !hasPic)
        {
            selectTargetFieldTask1Rep(clickedPic);
            willCorrectTask1Rep = false;
            canSwitchToNextTask = false;
            picSetted = false;
        }

        else if (!willCorrectTask1Rep && hasPic)
        {
            ...
            if (currentClass !== "selectedPic" && !picSetted)
            {
                // select pic
                $("#" + clickedPicId).attr("class", "selectedPic");
                picSelectedPart2Task1Rep = true;
                selectedPicIdPart2Task1Rep = clickedPicId;
            ...
            else
            {
                // deselect pic
                $("#" + clickedPicId).attr("class", "fieldBorder");
                picSelectedPart2Task1Rep = false;
                selectedPicIdPart2Task1Rep = null;
            }
...
function selectPicPart2Task1(clickedPic, choiceTableId) {
    ...
    try
    {
        ...
        else if (!willCorrectTask1 && hasPic)
        {
...
// select target field methods


// selects target Field during trial task
function selectTargetFieldTrial(clickedField, targetsTable) {

    var clickedFieldId = clickedField.id;
    willCorrect = $("#" + clickedFieldId).children('img').length > 0 && !picSelectedPart2Trial;
    var isEmpty = $("#" + clickedFieldId).children('img').length <= 0;

    try
    {
        // only if a pic was selected
        if (picSelectedPart2Trial && isEmpty)
        {
            // reset status
            picSelectedPart2Trial = false;
...
function selectTargetFieldTask1(clickedField, targetsTable) {

    var clickedFieldId = clickedField.id;
    willCorrectTask1 = $("#" + clickedFieldId).children('img').length > 0 && !picSelectedPart2Task1;
    var isEmpty = $("#" + clickedFieldId).children('img').length <= 0;

    try
    {
        // only if a pic was selected
        if (picSelectedPart2Task1 && isEmpty)
        {
            // reset status
            picSelectedPart2Task1 = false;
        ...
        // if user will correct selection
        else if (willCorrectTask1 && !isEmpty)
        {
            // highlight target field
            $("#" + clickedFieldId).attr("class", "selectedPic");
...
function selectTargetFieldTask1Rep(clickedField, targetsTable) {

    var clickedFieldId = clickedField.id;
    willCorrectTask1Rep = $("#" + clickedFieldId).children('img').length > 0 && !picSelectedPart2Task1Rep;
    var isEmpty = $("#" + clickedFieldId).children('img').length <= 0;

    try
    {
        // only if a pic was selected
        if (picSelectedPart2Task1Rep && isEmpty)
        {
            // reset status
            picSelectedPart2Task1Rep = false;
            ...
            setTimeout(function () {
                ...
                // not to same field
                if (selectedPicIdPart2Task1Rep !== clickedFieldId)
                {
                    $("#" + selectedPicIdPart2Task1Rep).children('img').clone().appendTo("#" + clickedFieldId);
                    $("#" + selectedPicIdPart2Task1Rep).find('img').remove();
                    // remove highlighting from selected pic and target field
                    $("#" + selectedPicIdPart2Task1Rep).attr("class", "fieldBorder");
                    $("#" + clickedFieldId).attr("class", "fieldBorder");
        ...
        // if user will correct selection
        else if (willCorrectTask1Rep && !isEmpty)
        {
            // highlight target field
            $("#" + clickedFieldId).attr("class", "selectedPic");

            selectedPicIdPart2Task1Rep = clickedFieldId;
            picSelectedPart2Task1Rep = true;
            canSwitchToNextTask = false;
        }
